...
import { createBooking } from "@/lib/appointments/actions";
...
export default async function BookPage({ searchParams }: { searchParams: Promise<{ slotId?: string; mrd?: string; leadId?: string }> }) {
  await requireCan("appointments", "create");
  const { slotId, mrd, leadId } = await searchParams;

  const [doctors, departments, branches, slot, lead] = await Promise.all([
  ...
  return (
    <div>
      <PageHeader title="Book appointment" subtitle={slot ? "From an open slot" : "Walk-in / manual"} />
      ...
      <form action={createBooking} className="mt-4 grid max-w-2xl grid-cols-2 gap-4">
        {leadId && <input type="hidden" name="leadId" value={leadId} />}

        <label className="col-span-2 text-sm font-medium text-slate-700">
          Patient MRD *
          <input name="patientMrd" required defaultValue={mrd ?? ""} placeholder="MRD-xxxxxxxx" className={input} />
