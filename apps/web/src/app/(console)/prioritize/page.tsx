              ...
              <tbody>
                ...
                {ranked.slice(0, 30).map(({ l, p }) => (
                  <tr key={l.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-2"><Link href={`/leads/${l.id}`} className="font-medium text-rose-700 hover:underline dark:text-rose-300">{l.contactName}</Link><div className="text-xs text-slate-400">{l.phone}</div></td>
            ...
            <ul className="space-y-1 text-sm">
              {highRisk.map((s) => (
                <li key={s.patientMrd} className="flex items-center justify-between">
                  <Link href={`/patients/${encodeURIComponent(s.patientMrd)}`} className="text-rose-700 hover:underline dark:text-rose-300">{s.patient.name}</Link>
