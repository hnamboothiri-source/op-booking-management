...
import { getModuleMaster, getCustomRecord, updateCustomRecord } from "@/lib/config/recordActions";
...
export default async function EditCustomRecord({ params }: { params: Promise<{ slug: string; mkey: string; id: string }> }) {
  const { slug, mkey, id } = await params;
  ...
  if (!master) notFound();
  const row = await getCustomRecord(id);
  if (!row) notFound();
  ...
  return (
    <div>
      ...
      <MasterForm master={synthetic} row={row} action={updateCustomRecord.bind(null, slug, mkey, id)} submitLabel="Save changes" />
    </div>
  );
}
